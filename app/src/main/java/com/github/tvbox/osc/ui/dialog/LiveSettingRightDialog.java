...
/**
 * 直播设置弹窗（右侧抽屉样式，全屏播放器设置入口）。
 * 内容编排全部委托 {@link LiveSettingPanel}；本类只保留 XPopup 抽屉壳差异。
...
public class LiveSettingRightDialog extends AppDrawerPopupView {
    ...
    public LiveSettingRightDialog(@NonNull @NotNull Context context, @NonNull LiveSettingHost host) {
        super(context);
        mHost = host;
    }

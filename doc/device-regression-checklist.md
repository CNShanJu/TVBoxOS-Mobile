...
> 2026-09-06 typed-first 收敛进展(评估 §J,饭太硬订阅/糯米源):detail/search/quickSearch/category(list)
> 四入口 type0/1/4 typed-first 已实施(旧路径兜底)并真机冒烟——糯米源搜索出结果、进详情选集/线路渲染、
> 播放出画面、主页/热播分类正常、无崩溃;type3 JS 源无回归;category 分页与 Home sort 待设备批逐源回归。
> 以下逐项待完整逐源回归:
...
> **2026-09-06 人工观感轮(用户实测)**:其余观感项(IJK/Exo 播放控制手感、切集、完成、
> 出错路径、断点续播、后台播放通知)均实测正常、无明显差异 → 本组全项通过。
- [x] IJK(play_type=1):play/pause/seek/进度回传、切集、播放完成、出错,与 doikki 直驱路径并行一致(起播已验证 ✓;暂停经 play_status 生效——事件流归零 ✓;恢复/seek/切集/完成/出错人工实测通过 ✓)
  - 切集交互冒烟(多集剧"现在不是出轨的问题"):点选集 01↔02,选中态切换正常、无崩溃;
    02 集触发新 playUrl;切集后自动续播依赖源线路解析,人工实测通过 ✓
- [x] Exo(play_type=2):同上(起播已验证 ✓;人工实测通过 ✓)
...
- [x] 会话 bind/release 无泄漏(进出详情/连续切集/页面销毁),共享 mVideoView 不被会话误释放
  - 首轮冒烟:连续进出详情 2 次,IJK 每次重新起流(线程 21024→21593,onNativeInvoke 正常),
    无崩溃/ClassNotFound/FATAL,共享视图可复用 ✓;人工实测反复进出/切集/销毁无泄漏异常 ✓
    (bind/unbind 精确配对日志因机型滤 D 级不可见,以"无泄漏可观测行为"为准,见下注)
...
- [ ] 旧版(含 Hawk 旧键)升级 → **不适用(mbox 包名隔离,见上注)** —— 结论已定,非缺陷
- [x] 升级后旧键清除;再次冷启不重复迁移、无异常日志 → mbox 新装冷启无 KeyValueStore/hawk 报错 ✓
- [x] 版 N(KeyValueStore 无 hawk 化)在"已迁移设备"上行为与升级前一致 → mbox 无此场景;设置持久化冷启生效 ✓
> **2026-09-06 收尾**:G/H 两组结论归档——G 组双内核+观感项+MediaSession 媒体卡全部实测通过;
> H 组对 mbox 判定"不适用"(无 Hawk 存量场景)并记录未来 osc 发布线验证路径。
